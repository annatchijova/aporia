...
async function request<T>(path: string, options: RequestInit = {}, session?: Session): Promise<T> {
  ...
  if (contentType.includes('application/json')) {
    try { data = JSON.parse(raw) as T & { error?: string }; } catch { data = null; }
  }
